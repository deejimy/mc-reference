...
3. Résolvez chacune des cartes de boost de l'ennemi attaquant, une à la fois et dans l'ordre dans lequel elles ont été distribuées, en procédant de la manière suivante :  
...
5. L'attaque termine sa résolution et les types de capacités suivants se déclenchent dans l'ordre :  
    - Le mot‑clé `Riposte X` (si le personnage attaqué est toujours en jeu).  
    - Les capacités forcées avec les déclencheurs suivants (dans n'importe quel ordre) :  
