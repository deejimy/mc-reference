...
<span class="new">Les termes « max » et « maximum » imposent un maximum pour tous les exemplaires d’une carte (par titre) pour tous les joueurs.</span>  
...
- « 1 max par joueur » est spécifique à chaque joueur et restreint le nombre d’exemplaires de cette carte que chaque joueur peut contrôler en jeu en même temps.  
    - <span class="new">Un joueur ne peut pas prendre le contrôle d’un autre exemplaire d’une carte « 1 max par joueur » qu’il contrôle déjà.</span>  
- <span class="new">« 1 max par [élément de jeu] » restreint le nombre d’exemplaires de cette carte qui peuvent être attachés à chaque élément de jeu indiqué. (Par exemple, si une amélioration est « 1 max par allié », un allié ne peut avoir au maximum qu’une copie de cette amélioration attachée.)</span>  
- <span class="new">« 1 max par [instance] » restreint le nombre de fois où une capacité peut être déclenchée par une seule instance d’un effet déclencheur parmi tous les exemplaires de la carte possédant ce maximum. (Par exemple, si une capacité porte le texte « (Max 1 per event.) », une seule carte avec cette capacité peut être déclenchée par événement joué.)</span>  
- <span class="new">La parenthèse « (to a maximum of [valeur]) » dans une capacité impose un maximum pour cette capacité pendant la durée de cette capacité.</span>  
