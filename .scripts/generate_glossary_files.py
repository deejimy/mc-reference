...
def main():
    ...
    # Créer le fichier Glossaire.md
    glossaire_path = os.path.join(output_dir, 'Glossaire.md')
    glossary_entry = []
    ...
    for english_title, content in chapters:
        try:
            ...
            if french_title:
                ...
                # Créer un dossier avec la première lettre du titre pour organiser les fichiers
                ...
                # Clean content
                content = content.replace(english_title, '# '+ french_title +'\n\n', 1)
                content = content.replace('See also', '\n\nSee also')
                
                # mise en forme mkdocs: bullets..
                content = content.replace('•', '\n - ')
                content = content.replace('» »', '\n\t - ')
                # Assurer que les listes à puces ont un espace après le tiret
                ...
                # Créer tableau pour stocker les titres et liens pour le glossaire
                glossary_entry.append(f'[[{filename.replace(".md","")}|{french_title}]]  \n')
    ...
    # Créer le fichier Glossaire.md avec les liens vers les chapitres
    with open(glossaire_path, 'w', encoding='utf-8') as f:
        f.write("# Glossaire\n\n")
        # ajouter en chapitre la 1ère lettre de chaque entrée
        current_letter = ''
        for entry in sorted(glossary_entry):
            letter = entry[2].upper()  # La lettre est le 4ème caractère après '[['
